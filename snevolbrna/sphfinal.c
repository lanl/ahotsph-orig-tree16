...
update_intermediate(SPHbody *btab, int nobj, float dt_last, int flag, int *limit)
{
    ...
    float gshift, xmtheo, avokb, sfac, steps, h, mass;
	...
	mass=p->mass;
	gshift = p->gshift;
xxx can i subtract more of the above?
    }
    StopTimer(&sphEOS);
}
