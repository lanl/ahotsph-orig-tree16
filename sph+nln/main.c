...
static void SPHOutputA(SPHbody *btab, int nobj, const char *outname, int iter);
static void SPHOutput_strength(SPHbody *btab, int nobj, const char *outname, int iter);
static void WindOutput(
    SPHbody *btab, int nobj, windbody *windbtab, int windnobj, const char *outname, int iter);
static void ShortWindOutput(
    SPHbody *btab, int nobj, windbody *windbtab, int windnobj, const char *outname, int iter);
...
int main(int argc, char *argv[]) {
    ...
    */
            ...
            if (strlen(params.SPHdatafile) > 0 || params.do_restart) {
                ...
                if (params.do_sph) {
                    if (params.do_restart)
                        sprintf(iname, "%s_sph.restart", params.name);
                    else
                        sprintf(iname, "%s", params.SPHdatafile);
                ...
                if (params.has_grav_data) {
                    if (params.do_restart)
                        sprintf(iname, "%s.restart", params.name);
                    else
                        sprintf(iname, "%s", params.name);
                    sdfp = DarkRead(
                        iname, csdfp, (void **)&btab, &gnobj, &nobj, params.set_id, params.setpvel);
                } else {
                    gnobj = nobj = 0;
                ...
                if (params.do_winds) {
                    if (params.old_winds) {
                        sdfp = WindRead(iname, csdfp, &windbtab, &windgnobj, &windnobj);
                    } else
                        windgnobj = windnobj = 0;

                    if (params.const_winds || params.nonconst_winds || params.accreting_winds) {
                        if (MPMY_Procnum() == 0) {
                            ReadTemplate(params.template_name, (template_t **)&tempbtab, &tempnobj);
                    ...
                    if (params.nonconst_winds) {
                        if (MPMY_Procnum() == 0)
                            ReadWindData(params.winddata_name, (winddata_t **)&wdata, &wnobj);
                    }
                }
    ...
    if (params.do_Bmax)
        MACtype = BMAX_MAC;
    else if (params.do_BH)
        MACtype = BH_MAC;
    else if (params.do_Arel)
        MACtype = AREL_MAC;
    else if (params.do_grav)
    ...
    dt_last = params.dt;
    SPH_setup(
        NDIM, params.kernel_ncoef1, params.kernel_coef1, params.kernel_ncoef2, params.kernel_coef2);
    ...
    if (params.do_strength && params.make_brittle) {
        if (params.defects_table_exists) {
            ...
            read_defects_table(
                defects_sdfp, &(params.Nflaws), &flaw_actv_tbl, &flaw_actv_tbl_lookup);
             ...
             * other ranks. */
            if (MPMY_Procnum() == 0) {
                init_defects_table(SPHgnobj,
                                   params.Nflaws,
                                   &flaw_actv_tbl,
                                   &flaw_actv_tbl_lookup,
                                   params.material.material_k * params.material.Vol0,
                                   params.material.material_m);
                sprintf(params.defects_file, "%s_flaws.sdf", params.outnamebase);
                write_defects_table(params.defects_file,
                                    SPHgnobj,
                                    params.Nflaws,
                                    flaw_actv_tbl,
                                    flaw_actv_tbl_lookup);
    ...
    for (params.nsteps += iter; iter <= params.nsteps; iter++) {
        if (params.timeout > 0)
            MPMY_TimeoutReset(params.timeout);
        ...
        if (params.do_winds && params.const_winds) {
            ...
            /* Only proc 0 adds wind particles; would be better to add
               ...
               wind in the first place */
            dt_wind = 1e30;
            AddNonconstWinds((SPHbody **)&SPHbtab,
                             &SPHnobj,
                             tempbtab,
                             params.windpartpershell,
                             wdata,
        ...
        if (params.do_boundary) {
            SPHoldnobj = SPHnobj;
            AdjustBtab3((SPHbody **)&SPHbtab, &SPHnobj, SPHgnobj, params.r_inner, params.r_outer);
            MPMY_Combine(&SPHnobj, &SPHgnobj, 1, MPMY_INT, MPMY_SUM);
            Msgf(("Iter: %d: Removed %d bodies from SPHbtab\n", iter, SPHoldnobj - SPHnobj));
        }

        if (params.do_absorbing_bndry) {
            SPHoldnobj = SPHnobj;
            AdjustBtab4((SPHbody **)&SPHbtab,
                        &SPHnobj,
                        bndry,
                        &newmass,
                        &newr,
        ...
        if (params.do_grav && dark_need_update(dark_tacc, params.dark_dt)) {
            ...
            if (MACtype == AREL_MAC)
                this_tol = params.tol * mtot / (sysradius * sysradius);
        ...
        if (params.do_sph && (first_step || params.exact_rho)) {
            ...
            StartTimer(&BuildTot);
            pqsortsetup(
                &SPHsortedbtab, SPHbtab, SPHnobj, sizeof(SPHbody), params.sort_tol, Realloc_f);
        ...
        if (params.do_sph) {
            ...
            if (did_dark_update) {
                ...
                pqsortsetup(
                    &sortedatab, SPHatab, SPHanobj, sizeof(accbody), params.sort_tol, Realloc_f);
        ...
        if (params.do_point_mass2 || params.do_boundary) {
            update_point_SPHmass2(
                SPHbtab, SPHnobj, params.eps * params.eps, cosmo.GNewt, params.centmass);
        }
        ...
        if (ForceOutput()
                ...
                if (params.do_winds) {
                    if (params.old_winds) {
                        if (params.short_output)
                            ShortWindOutput(
                                SPHbtab, SPHnobj, windbtab, windnobj, params.outnamebase, iter);
                        else
                            WindOutput(
                                SPHbtab, SPHnobj, windbtab, windnobj, params.outnamebase, iter);
                    } else {
            ...
            if (params.has_grav_data)
                Output(btab, nobj, params.outnamebase, iter);
            if (params.do_point_mass)
                Output(pmtab, PMnobj, params.outnamebase, iter);
        }
        ...
        if (params.do_strength) {
            for (i = 0; i < NDIM * NDIM; i++) {
                ...
                ABUpdateXs(&SPHbtab[0].data.strengthbody.stress[i],
                           SPHstride,
                           &SPHbtab[0].data.strengthbody.dstressdt[i],
                           SPHstride,
                           &SPHbtab[0].data.strengthbody.dstressdt_last[i],
                           SPHstride,
                           &SPHbtab[0].ident,
                           SPHstride3,
                           SPHnobj,
            ...
            for (i = 0; i < SRTERMS; i++) {
                ABUpdateXs(&SPHbtab[0].data.strengthbody.strain[i],
                           ...
                           SPHstride,
                           &SPHbtab[0].data.strengthbody.dstraindt_last[i],
                           SPHstride,
                           &SPHbtab[0].ident,
                           SPHstride3,
                           SPHnobj,
            ...
            UpdateSXs(&SPHbtab[0].data.strengthbody.dmg,
                      SPHstride,
                      &SPHbtab[0].data.strengthbody.ddmgdt,
                      SPHstride,
                      &SPHbtab[0].ident,
                      SPHstride3,
                      SPHnobj,
        ...
        tvel += dt;
        dt_last = dt;
        for (i = 0; i < SPHnobj; i++) { VV(SPHbtab[i].acc_last, = SPHbtab[i].acc); }
        if (params.cosmology) {
        ...
        if (params.adaptive_dt)
            Fix_dt(&dt,
                   (((params.dark_dt < params.dt_max) ? params.dark_dt : params.dt_max) < dt_wind)
                       ? ((params.dark_dt < params.dt_max) ? params.dark_dt : params.dt_max)
                       : dt_wind,
                   params.tlow_cut,
                   tmin,
                   tbad,
        ...
        if (params.SPHCWfac != 0.0) {
            ...
            gnterms *= params.SPHCWfac;
            for (q = SPHbtab; q < SPHbtab + SPHnobj; ++q) q->nterms += gnterms;
...
static void WindOutput(
    SPHbody *btab, int nobj, windbody *windbtab, int windnobj, const char *outnamebase, int iter) {
    SPHbody *p;
    int i;
...
static void ShortWindOutput(
    ...
    singlPrintf("Trying to sort output\n");
    pqsortsetup_order(
        &outputsort, output_btab, output_nobj, sizeof(SPHshortoutbody), 0.1F, 1, Realloc_f);
...
static void SPHOutput(SPHbody *btab, int nobj, const char *outnamebase, int iter) {
    ...
    for (i = 0; i < output_nobj; i++) {
        ...
        output_btab[i].Y_el = btab[i].data.nucnetw.Y_el;
        output_btab[i].mfp = btab[i].data.nucnetw.mfp;
    }
...
static void SPHOutputA(SPHbody *btab, int nobj, const char *outnamebase, int iter) {
    SPHbody *p;
    int i, j;
    sortresult_t outputsort;
    ...
    singlPrintf("Trying to sort output\n");
    pqsortsetup_order(
        &outputsort, output_btab, output_nobj, sizeof(SPHoutbody_NW), 0.1F, 1, Realloc_f);
    output_btab = pqsort(&outputsort, UnityCost, (pq_keyproto)SPHOutIdentKey);
    output_nobj = outputsort.nobj;
    /*     Msg("output", ("After pqsort, %d outbodies\n", output_nobj)); */
    Msgf(("After pqsort, %d outbodies\n", output_nobj));
...
static void SPHOutput_strength(SPHbody *btab, int nobj, const char *outnamebase, int iter) {
    ...
    singlPrintf("Trying to sort output\n");
    pqsortsetup_order(
        &outputsort, output_btab, output_nobj, sizeof(SPHoutbody_strength), 0.1F, 1, Realloc_f);
    output_btab = pqsort(&outputsort, UnityCost, (pq_keyproto)SPHOutIdentKey);
    output_nobj = outputsort.nobj;
    /*     Msg("output", ("After pqsort, %d outbodies\n", output_nobj)); */
    Msgf(("After pqsort, %d outbodies\n", output_nobj));
...
static void SPHDiags(SPHbody *btab,
    ...
    for (p = btab; p < btab + nobj; p++) {
        ...
        if (!(p->ident & (1 << 30))) { /* If not a boundary particle... */
            ...
            if (params.limit_dt_on_acc && tx < dti)
                dti = tx;
            ...
            if (dti > params.dt_max && 2.0 * p->dt < params.dt_max)
                p->dt_next = 2.0 * p->dt;
            ...
            if (dti < dt)
                tlow++;
            if (p->dt_next > params.dt_max)
                p->dt_next = params.dt_max;
            if (p->rho > max_rho)
                max_rho = p->rho;
            if (p->rho < min_rho)
                min_rho = p->rho;
            ...
            if (p->u < 0.0) {
                SeriousWarning(
                    "Iter %d: particle has negative energy\n%s\n", iter, PrintSPHBodyContents(p));
                p->u = 0.0;
            }
