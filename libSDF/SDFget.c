...
#define SDFget(sdfp, name, addr)                        \
    if (SDFseekrdvecs(sdfp, name, 0, 1, addr, 0, NULL)) \
    return -1
    ...
    switch (SDFtype(name, sdfp)) {
        ...
        case SDF_INT:
            ...
            *value = (float)int_value;
            return 0;
        default:
            sprintf(SDFerrstring, "SDFgetfloat: \"%s\" must be either float or int.\n", name);
            return -1;
    }
}
    ...
    switch (SDFtype(name, sdfp)) {
        ...
        case SDF_INT:
            ...
            *value = (double)int_value;
            return 0;
        default:
            sprintf(SDFerrstring, "SDFgetdouble: \"%s\" must be either float or int.\n", name);
            return -1;
    }
}
    ...
    switch (SDFtype(name, sdfp)) {
        ...
        case SDF_DOUBLE:
            ...
            if ((double)int_value != double_value) {
                sprintf(SDFerrstring, "SDFgetint: \"%s\" has lost precision\n", name);
                return -1;
            }
            *value = int_value;
            return 0;
...
int SDFgetstring(SDF *sdfp, const char *name, char *string, int size) {
    ...
    if (string[len - 1] != '\0') {
        sprintf(SDFerrstring, "Read non-terminated string in SDFgetstring(\"%s\")\n", name);
        string[len] = '\0';
    }
    return ret;
}
