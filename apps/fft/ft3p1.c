...
fourt(fcomplex data[], unsigned long nn[], int ndim, int isign)
{
	...
	if (Nproc() == 1) {
	    ...
	    transpose_sort(Procnum(), buf, xp*sizeof(fcomplex));
	    memcpy(q, buf, nn[0]*sizeof(fcomplex));
	    Free(buf);
	}
	...
	if (Nproc() == 1) {
		...
		vcopy(p+xn*sendproc, (fcomplex *)buf+xp*sendproc, xp, xnum, 1);
	    }
	    Free(buf);
	}
    }
