...
/* This file gives fits to the higher levels of HPUX optimization.  We just
   turn it off for now. */
#pragma OPTIMIZE OFF
#endif
...
int MPMY_ICombine_func(const void *sendbuf,
                       void *recvbuf,
                       int size,
...
void MPMY_Combine_Mask(unsigned int maskval) { mask = maskval; }


/* gcc with optimization will cause this to fail efence */
#define Do_Op(outbuf, op, inbuf, type, cnt)                                   \
    do {                                                                      \
        char *oend = (char *)outbuf + StkAlign(&outdata, cnt * sizeof(type)); \
...
static void do_combine(const void *inbuf,
                       void *outbuf,
                       const int n,
...
int MPMY_ICombine_Wait(MPMY_Comm_request req) {
    ...
    for (i = 0; i < n; i++) {
        ...
        manifest
            = (const struct comb_st *)((char *)manifest + StkAlign(&outdata, sizeof(*manifest)));
    }
    Free(inbuf);
