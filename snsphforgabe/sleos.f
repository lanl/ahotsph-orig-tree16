...
cc        IF((U_NUC.LT.0.0).OR.((U_NUC-1.0).GT.-1.0E-20)) THEN
        IF((U_NUC.LE.0.0).OR.(U_NUC.GT.0.996)) THEN
          NGFLAG = 1
          GOTO 29
        ENDIF
C
...
C
C
C      IF(NSUBI.LT.0.05) THEN
C        WRITE(*,*) 'NUCEOS:: <<WARNING>> NSUBI GETTING CLOSE TO LB'
C      ENDIF
C
C
C
...
C
C       WRITE(*,*) ' EOSLOG:: Cant do Ye = ',YE, 'at this time'
C       WRITE(*,*) ' EOSLOG:: assuming YE =',Y_HI,' instead'
        YE = Y_HI
        GOTO 10
C
...
C Following vars were undefined due to goto.  Fixed 5/26/2001 by mswarren
	LMM=0.
	LMP=0.
