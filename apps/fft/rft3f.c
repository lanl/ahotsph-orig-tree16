...
main(int argc, char *argv[])
{
    ...
    fp = MPMY_Fopen("rft3in",  MPMY_RDONLY | MPMY_MULTI);

    MPMY_Fread(data1, ndat2, sizeof(fcomplex), fp);
    MPMY_Fclose(fp);
...
fourP(fcomplex data[], unsigned long nn[], int ndim, int isign)
{
	...
	tptr = data;
	buf = direct_untranspose();
	for (sendproc = 0; sendproc < nproc; sendproc++) {
	    vvcopy(data+xn*sendproc, buf+xp*xn*sendproc, xp, xn, xnum, xn);
	}
	Free(buf);
