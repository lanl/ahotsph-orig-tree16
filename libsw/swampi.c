...
#include <xti.h> /* Has TCP_NODELAY and TCP_MAXSEG */
#endif
...
static unsigned int MPI_Datasize[] = {
    ...
    sizeof(MPI_float_int),
    sizeof(MPI_double_int),
    sizeof(MPI_long_int),
    sizeof(MPI_2int),
    sizeof(MPI_short_int),
    sizeof(MPI_long_double_int),
    2 * sizeof(float),
    2 * sizeof(double),
    1 /*user_data*/
};
...
char *mpi_datatype_name[_MPI_NUMDATATYPES]
    = {"float",           "double",        "long double",    "byte",          "char",
       "short",           "int",           "long",           "long long",     "unsigned",
       "unsigned int",    "unsigned char", "unsigned short", "unsigned long", "unsigned long long",
       "float int",       "double int",    "long int",       "2int",          "short int",
       "long double int", "complex",       "double complex", "user data"};

/* Must match MPI_Op enum */
char *mpi_op_name[_MPI_NUMOPS] = {
    "sum", "prod", "max", "min", "band", "bor", "bxor", "land", "lor", "lxor", "maxloc", "minloc"};
...
char *mpi_stats_name[_MPI_NUMSTATS] = {"Exec Time",
                                       "Wait Time",
                                       "Test Time",
                                       "Select Time",
                                       "Send Time",
...
#define MAX_NPOST 100     /* This many req_t ptrs for each proc */
...
#define PKTSIZE (10 * 1460)

static double hwtick;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if ((m = strstr(line, "cpu MHz"))) {
            ...
            if (nscan == 1) {
                ...
                Warning(
                    "sscanf returns %d, on '%s' of /proc/cpuinfo mhz line failed\n", nscan, line);
                fclose(fp);
...
double hwclock(void) {
    ...
    if (init) {
        init = 0;
        hwtick = 1.0 / mhz_from_proc();
        hwclock_offset = hwtick * ((double)counter[1] * two_to_32 + (double)counter[0]);
        return 0.0;
    }
    return (hwtick * ((double)counter[1] * two_to_32 + (double)counter[0]) - hwclock_offset);
}
#endif

#ifdef __alpha__
#define ALPHA_CLOCK 533e6
double hwclock(void) {
    static int init = 1;
    unsigned long cc;
    unsigned int lo;
    static unsigned int last_lo;
    ...
    if (init) {
        ...
        hi = 0;
        last_lo = 0;
        hwclock_offset = hwtick * ((double)hi * two_to_32 + (double)lo);
        return 0.0;
    }
    if (lo < last_lo) {
        hi++;
    ...
    last_lo = lo;
    return (hwtick * ((double)hi * two_to_32 + (double)lo) - hwclock_offset);
}
#endif
#else
double hwclock(void) {
    ...
    if (init) {
        init = 0;
        gettimeofday(&tp, &tzp);
        hwclock_offset = (double)tp.tv_sec + (double)tp.tv_usec * 1.e-6;
        return 0.0;
    }
    gettimeofday(&tp, &tzp);
    return ((double)tp.tv_sec + (double)tp.tv_usec * 1.e-6 - hwclock_offset);
}
#endif

static void zero_hwclock(void) { hwclock_offset += hwclock(); }
...
static void init_elt(void) {
    ...
    if (!(getenv("MPI_PROCNUM") && getenv("MPI_NPROC") && getenv("MPI_HOSTPORT")
          && getenv("MPI_HOST")))
    ...
    if (getenv("MPI_MESSAGE_TURNON") && strlen(getenv("MPI_MESSAGE_TURNON")) > 0) {
    ...
    sprintf(MPMY_Abchdir_arg, "mpi.%02d", MPI_Procnum);
    ...
    if (read_active == NULL || write_active == NULL || recv_list == NULL || send_list == NULL)
        Error("out of memory\n");
...
void _MPI_init_host(int n) /* n is how many nodes we talk to */
{
    int i;
    int proc;
    unsigned int addr_len = sizeof(struct sockaddr_in);
...
static void sock_setopt(int fd) {
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_NO_CHECK, (void *)&no_check, sizeof(int)))
        Error("sockopt no_check, errno=%d", errno);
#endif
    if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (void *)&sendbuf, sizeof(int)))
        Error("sockopt sndbuf, errno=%d", errno);
    if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (void *)&recvbuf, sizeof(int)))
...
static char *printSockaddr(const struct sockaddr_in *sa) {
    static char ans[512];
    sprintf(ans,
            "sockaddr_in: family: %d, sin_addr: %s, sin_port is %d\n",
            sa->sin_family,
...
static int sock_connect(struct sockaddr_in *acc) {
    ...
    if (ret) {
        ...
        again:
            if (ret < 0) {
                if (errno == EINTR)
                    goto again; /* SIGPROF interrupts select */
                else
...
static int sock_bind(struct sockaddr_in *acc) {
    ...
    if (ret < 0)
        Error(" Can't bind socket. errno=%d\n", errno);
...
static void req_done(Dll_elmt *elmt, MPI_Status *status) {
    ...
    if (req->outgoing) {
        ...
        if (req->buffered)
            remove = bufr_list;
        else if (req->wild_src)
            remove = wild_list;
        else
            remove = recv_list + req->proc;
    }
    DllDelete(remove, elmt);
    len = DllLength(remove);
    if (len < 0 || len > MAX_NPOSTANY)
        Error("bad dll length (%d)\n", len);
}
...
static void spin_io(Dll_elmt *elmt) {
    int i, j, ret;
    fd_set rdset, wtset;
    ...
    if (elmt) {
        ...
        Msgf(("mpi: %9f spin_io %s (%d.%d) %d \n",
              hwclock(),
              (req->outgoing) ? "sending" : "receiving",
              req->tag,
              req->comm,
              req->proc));
    ...
    do {
        ...
        for (i = 0; i < MPI_Nproc; i++) {
            ...
            if (read_active[i] || wild_pending != NULL || io_pending(recv_list + i) != NULL)
                FD_SET(s[i], &rdset);
...
static int do_io(req_t *req, int proc) {
    ...
    if (!req->hdr_flag) {
        ...
        if (req->outgoing) {
            ...
            t1 = hwclock();
            if (writemsg(fd, pktbuf, n + sizeof(msghdr_t)) == -1)
                return 0;
            t2 = hwclock();
            ...
            if (readmsg(fd, &hdr, sizeof(msghdr_t)) == -1)
                return 0;
    ...
    if (req->pending == 0)
        Error("req already completed\n");
    if (req->proc != proc)
        Error("req is inconsistent with proc\n");
    left = MIN(req->left, PKTSIZE);
    /* Small messages get sent with the header */
    if (left) {
        t1 = hwclock();
#if 1
        n = (req->outgoing) ? write(fd, req->ptr, left) : read(fd, req->ptr, left);
#else
        n = (req->outgoing) ? send(fd, req->ptr, left, 0) : recv(fd, req->ptr, left, 0);
#endif
        t2 = hwclock();
        if (n < 0) {
            if (errno == EAGAIN)
    ...
    if (req->left == 0) {
        req->pending = 0;
        if (req->outgoing)
            write_active[proc] = NULL;
        else
            read_active[proc] = NULL;
    }
    return n;
}

static int writemsg(int fd, const void *p, int n) {
    int nwrote;
    int left = n;
    const char *ptr = p;
    while (left > 0) {
...
static int readmsg(int fd, void *p, int n) {
    int nread;
    int left = n;
    char *ptr = p;
    while (left > 0) {
...
static void writemsg_block(int fd, const void *p, int n) {
    int nwrote;
    int left = n;
    const char *ptr = p;
    while (left > 0) {
...
static void readmsg_block(int fd, void *p, int n) {
    int nread;
    int left = n;
    char *ptr = p;
    while (left > 0) {
...
static req_t *match_tag(msghdr_t *hdr, Dll *wildp, Dll *recvp) {
    Dll_elmt *p;
    req_t *req;
...
req_t *parse_hdr(msghdr_t *hdr, req_t *req, int proc) {
    ...
    if (hdr->src != req->proc) {
        if (req->wild_src)
            ...
            Error("bad src, got %d expected %d\n", hdr->src, req->proc);
    }
    if (hdr->len != req->len) {
        if (hdr->len >= 0 && hdr->len < req->len)
            req->len = req->left = hdr->len;
        else
            Error("bad len, got %d expected %d\n", hdr->len, req->len);
    }
...
static void mpi_diagnostics(void) {
    ...
    sum = 0;
    for (i = 0; i < MPI_Nproc; i++) sum += DllLength(send_list + i);
    if (sum) {
        Msg_do("mpi: %d writes active:\n", sum);
        for (i = 0; i < MPI_Nproc; i++) dump_req(send_list + i);
    }
    sum = 0;
    for (i = 0; i < MPI_Nproc; i++) sum += DllLength(recv_list + i);
    if (sum) {
        Msg_do("mpi: %d reads active:\n", sum);
        for (i = 0; i < MPI_Nproc; i++) dump_req(recv_list + i);
    }
...
double MPI_Wtime(void) { return hwclock(); }

double MPI_Wtick(void) { return hwtick; }
...
int MPI_Irecv(
    ...
    for (elmt = DllBottom(bufr_list); elmt != DllSup(bufr_list); elmt = DllUp(elmt)) {
        request = DllData(elmt);
        if ((tag == request->tag || tag == MPI_ANY_TAG)
            && (src == request->proc || src == MPI_ANY_SOURCE) && comm == request->comm) {
...
int MPI_Test(MPI_Request *rptr, int *flag, MPI_Status *status) {
    Dll_elmt *elmt = *rptr;
    req_t *req;
    double t1;

    if (elmt == NULL)
        Error("NULL message request\n");
    req = DllData(elmt);
...
int MPI_Wait(MPI_Request *rptr, MPI_Status *status) {
    Dll_elmt *elmt = *rptr;
    req_t *req;
    double t1;
    ...
    t1 = hwclock();
    while (req->pending) spin_io(elmt);
    t1 = hwclock() - t1;
...
int MPI_Waitall(int count, MPI_Request *reqv, MPI_Status *statusv) {
    ...
    for (i = 0; i < count; i++)
        if (statusv == NULL)
            MPI_Wait(reqv + i, NULL);
        else
            MPI_Wait(reqv + i, statusv + i);
    return MPI_SUCCESS;
}
...
int MPI_Barrier(MPI_Comm comm) {
    ...
    if (MPI_Procnum != 0) {
        ...
        for (i = 1; i < MPI_Nproc; i++) MPI_Send(&junk, 1, MPI_INT, i, 2, MPI_COMM_PRIVATE);
        free(req);
    }
    return MPI_SUCCESS;
}
...
int MPI_Alltoall(void *sendbuf,
    ...
    if (rreq == NULL || sreq == NULL || status == NULL)
        Error("out of memory\n");
    for (i = 0; i < MPI_Nproc; i++) {
        MPI_Irecv(rbuf + i * recvcount * MPI_Datasize[recvtype],
                  recvcount,
                  recvtype,
                  i,
                  TAG,
    ...
    for (i = 0; i < MPI_Nproc; i++) {
        MPI_Isend(sbuf + i * sendcount * MPI_Datasize[sendtype],
                  sendcount,
                  sendtype,
                  i,
                  TAG,
    ...
    for (i = 0; i < MPI_Nproc; i++) { MPI_Wait(&rreq[i], &status[i]); }
    for (i = 0; i < MPI_Nproc; i++) { MPI_Wait(&sreq[i], NULL); }
    free(status);
    free(sreq);
    free(rreq);
    return MPI_SUCCESS;
...
int MPI_Alltoallv(void *sendbuf,
    ...
    if (rreq == NULL || sreq == NULL || status == NULL)
        Error("out of memory\n");
    for (i = 0; i < MPI_Nproc; i++) {
        MPI_Irecv(rbuf + rdispls[i] * MPI_Datasize[rtype],
                  recvcnts[i],
                  rtype,
                  i,
                  TAG,
    ...
    for (i = 0; i < MPI_Nproc; i++) {
        MPI_Isend(sbuf + sdispls[i] * MPI_Datasize[stype],
                  sendcnts[i],
                  stype,
                  i,
                  TAG,
...
/* The MPI Fortran binding requires MPI_Request to fit in an int */
/* which is problematical for 64-bit architectures */

#define f_req_SIZE 1024
#define f_req_FREE NULL

static int f_req_n = 1;
static int f_req_outstanding;
static MPI_Request f_req[f_req_SIZE];

typedef int *f_MPI_Request;
