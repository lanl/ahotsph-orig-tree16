...
int main(int argc, char *argv[]) {
        ...
        if (!((strncmp(name, "test", 4) == 0))) {
            if (SDFhasname("SPHdatafile", csdfp) || do_restart) {
                ...
                if (do_winds) {
                    ...
                    SDFgetintOrDie(csdfp, "windpart_per_shell", &windpartpershell);
                } else
    ...
    for (nsteps += iter; iter <= nsteps; iter++) {
        ...
        if (do_point_mass2 || do_plummer) {
            update_point_SPHmass3(SPHbtab, SPHnobj, eps * eps, cosmo.GNewt, centmass, cosmo.b);
        }

        if (do_bardeen) {
            update_point_mass_bardeen(SPHbtab, SPHnobj, cosmo.GNewt, centmass, bh_rplus, bh_S);
        }
        ...
        if (ForceOutput() || (do_output && !first_step && ((iter + output_freq) % output_freq == 0))
            ...
            || (wc_limit && ((double)wc_limit - tottime < 45.0 + 3.0 * StepTotWC.max))) {
            if (do_sph) {
                if (do_winds) {
                    if (short_output)
                        ShortWindOutput(SPHbtab, SPHnobj, windbtab, windnobj, outnamebase, iter);
                    else
                        WindOutput(SPHbtab, SPHnobj, windbtab, windnobj, outnamebase, iter);
                } else
        ...
        if (wc_limit && ((double)wc_limit - tottime < 45.0 + 3.0 * StepTotWC.max)) {
