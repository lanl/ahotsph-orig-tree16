...
int Msg_nopen(void) { return nfiles; }
...
int Msg_doalist(const char *fmt, va_list alist) {
    int i;
    struct ft_s *ft;
    int save;
    int extra = 0;
...
void Msg_turnon(const char *msg_turn_on) {
    ...
    /* empty string. */
    if (msg_turn_on == NULL || msg_turn_on[0] == '\0' || strcmp(msg_turn_on, "nomsgs") == 0) {
        Msg_set_enable(0);
        return;
    }
    ...
    for (msg_key = strtok(copy, " ,\t\n"); msg_key; msg_key = strtok(NULL, " ,\t\n")) {
        ...
        if (strncmp(__FILE__, "./", 2) == 0 && strncmp(msg_key, "./", 2) != 0) {
