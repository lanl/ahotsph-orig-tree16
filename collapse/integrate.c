...
void SetBoundary(int bound) { boundary_bit = bound; }
...
void ABUpdateXs(float *x,
                ...
                float h) {
    float *end = x + n * xstride;
    double c1 = dt * ((double)1.0 + (double)0.5 * dt / h);
    double c2 = (double)0.5 * dt * dt / h;
...
void PUpdateXsd(double *x,
    ...
    double *end = x + n * xstride;
    double tmp[NDIM];
    double half_dt2_plus_hdt = (double)0.5 * dt * (dt + h);
    double dt_h = dt / h;
