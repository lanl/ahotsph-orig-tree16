...
/* With optimization on, we seem to get spurious SEGFAULTS.  Why?  I dunno. */
#pragma OPTIMIZE OFF
#endif
...
extern void _obstack_begin_1(struct obstack *, int, int, void *(*)(), void (*)(), void *);
#else
...
#define obstack_specify_allocation_with_arg(h, size, alignment, chunkfun, freefun, arg) \
    _obstack_begin_1(                                                                   \
        (h), (size), (alignment), (void *(*)())(chunkfun), (void (*)())(freefun), (arg))
...
#define obstack_grow(OBSTACK, where, length)                                                    \
    __extension__({                                                                             \
        struct obstack *__o = (OBSTACK);                                                        \
        int __len = (length);                                                                   \
        ((__o->next_free + __len > __o->chunk_limit) ? (_obstack_newchunk(__o, __len), 0) : 0); \
...
#define obstack_grow0(OBSTACK, where, length)                                                      \
    __extension__({                                                                                \
        struct obstack *__o = (OBSTACK);                                                           \
        int __len = (length);                                                                      \
        ((__o->next_free + __len + 1 > __o->chunk_limit) ? (_obstack_newchunk(__o, __len + 1), 0)  \
                                                         : 0),                                     \
            bcopy(where, __o->next_free, __len), __o->next_free += __len, *(__o->next_free)++ = 0; \
        (void)0;                                                                                   \
    })
...
#define obstack_1grow(OBSTACK, datum)                                                   \
    __extension__({                                                                     \
        struct obstack *__o = (OBSTACK);                                                \
        ((__o->next_free + 1 > __o->chunk_limit) ? (_obstack_newchunk(__o, 1), 0) : 0), \
            *(__o->next_free)++ = (datum);                                              \
        (void)0;                                                                        \
    })
...
#define obstack_finish(OBSTACK)                                                               \
    __extension__({                                                                           \
        ...
        __o1->next_free = __INT_TO_PTR((__PTR_TO_INT(__o1->next_free) + __o1->alignment_mask) \
                                       & ~(__o1->alignment_mask));                            \
        ((__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk)    \
...
#define obstack_grow(h, where, length)                                                         \
    ((h)->temp = (length),                                                                     \
     (((h)->next_free + (h)->temp > (h)->chunk_limit) ? (_obstack_newchunk((h), (h)->temp), 0) \
                                                      : 0),                                    \
...
#define obstack_1grow(h, datum)                                                      \
    ((((h)->next_free + 1 > (h)->chunk_limit) ? (_obstack_newchunk((h), 1), 0) : 0), \
     *((h)->next_free)++ = (datum))
...
#define obstack_int_grow(h, datum)                                                                 \
    ((((h)->next_free + sizeof(int) > (h)->chunk_limit) ? (_obstack_newchunk((h), sizeof(int)), 0) \
                                                        : 0),                                      \
...
#define obstack_blank(h, length)                                                               \
    ((h)->temp = (length),                                                                     \
     (((h)->chunk_limit - (h)->next_free < (h)->temp) ? (_obstack_newchunk((h), (h)->temp), 0) \
                                                      : 0),                                    \
     (h)->next_free += (h)->temp)
...
#define obstack_finish(h)                                                               \
    (((h)->next_free == (h)->object_base ? (((h)->maybe_empty_object = 1), 0) : 0),     \
     ...
     (h)->next_free = __INT_TO_PTR((__PTR_TO_INT((h)->next_free) + (h)->alignment_mask) \
                                   & ~((h)->alignment_mask)),                           \
     (((h)->next_free - (char *)(h)->chunk > (h)->chunk_limit - (char *)(h)->chunk)     \
...
#define obstack_free(h, obj)                                                          \
    ((h)->temp = (char *)(obj) - (char *)(h)->chunk,                                  \
     (((h)->temp > 0 && (h)->temp < (h)->chunk_limit - (char *)(h)->chunk)            \
          ? (int)((h)->next_free = (h)->object_base = (h)->temp + (char *)(h)->chunk) \
