...
/*    opacity = KBF_COEFF*(1. - p->data.nucnetw.abund[18] - p->data.nucnetw.abund[19])*(1.+ p->data.nucnetw.abund[18]);*/
    /* free-free opacity */
/*    opacity = (opacity + KFF_COEFF*(p->data.nucnetw.abund[18]+p->data.nucnetw.abund[19])*(p->data.nucnetw.abund[18]+1.0))*
...
/*    mfp = 1.0/opacity * ivlenCF; *//* also convert to code units. here or next line? */
    ...
    for( j = 0; j < NNW; j++)
        eos_n += ((double)(rho))*N_AVOG /
                (double)(nparr[j] + nnarr[j]) * p->data.nucnetw.abund[j];
...
float cooling(SPHbody *p, float dt, float frac, int Gridpts, int Nel, int *notprinted) {
    ...
    if(p->data.nucnetw.mfp < 2.*p->h) return 0.;
