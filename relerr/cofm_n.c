...
void CofmFromDaugh(hcellptr hptr, hcellptr daughters[]){
	...
	if( Sub_Flags(daughters[i]) == 0 ){
	    bp = daughters[i]->ptr;
	    assert(bp);	    cmp->ndaughters++;
	    dmass = bp->mass;
	    acc_last = bp->acc_last;
	    cmp->mass += dmass;
	    ...
	    dmass = dp->mass;
	    acc_last = dp->acc_last_max;
	    cmp->mass += dmass;
	    VV(cmp->pos, += dmass * dp->pos);
	} if (acc_last < cmp->acc_last_max)
	    cmp->acc_last_max = acc_last;
    }
...
void CellFromCofm(cell *cp, cofmdata *cmp){
    ...
    cp->bmax = cmp->bmax;
    cp->acc_last_max = cmp->acc_last_max;
