...
c--figure out neutrino degeneracy 
         call rooteta(tmev,rhoi,ynuei,unuei,etanuei,tempnuei)
         ediff=enueti
...
c--de Broglie wavelength fm
      ...
      if (enuebti.gt.5.) then
         call rooteta(tmev,rhoi,ynuebi,unuebi,etanuebi,tempnuebi)
         ediff=enuebti
...
c--de Broglie wavelength fm
      ...
      if (enuxti.gt.5.) then
         call rooteta(tmev,rhoi,ynuxi,.25*unuxi,etanuxi,tempnuxi)
         ediff=enuxti
...
c--   bnueb: anti-neutrino end-state blocking
            ...
            expon=exp((tprot-tfne)/tempi)
c--bne: neutron end-state blocking
            bne=expon/(1.+expon)
            expon=exp((tneut-tfpr)/tempi)
...
c
      ...
      subroutine rooteta(tmev,rho,ynu,unu,eta,temp)
c***********************************************************
c
...
c
      if (ri.lt.rmaxnux.and.abs(ynuxi).gt.10.) then 
         write(*,100) ri,rmaxnux,ynuxi,dynuxi,dunuxi
      end if
