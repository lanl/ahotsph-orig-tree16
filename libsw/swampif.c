...
void _F77(mpi_isend)(void *buf, int *cnt, int *type, int *dest, int *tag,
		     int *comm, f_MPI_Request req, int *ierr)
{
    if (f_req_outstanding++ >= f_req_SIZE)
      Error("No more f_reqs avaliable\n");
    while (f_req[f_req_n] != f_req_FREE) {
...
void _F77(mpi_waitall)(int *count, f_MPI_Request reqv, 
		       MPI_Status *statusv, int *ierr)
{
    int i;

    for (i = 0; i < *count; i++) {
      if (statusv == NULL) MPI_Wait(&f_req[reqv[i]], NULL);
      else MPI_Wait(&f_req[reqv[i]], statusv+i);
