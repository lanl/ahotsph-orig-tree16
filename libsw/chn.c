...
void ChnInit(Chn *new, int sz, int nalloc, void *(*realloc_like)(void *, size_t)) {
    if (sz < ChnHDRSZ)
        sz = ChnHDRSZ;
...
int ChnMoreMem(Chn *id) {
    ...
    nnew = id->nalloc;
    sz = id->sz;
    while ((newchunk = id->realloc_like(NULL, nnew * sz + ChnHDRSZ)) == 0 && nnew > 0) nnew /= 2;
    if (newchunk == 0) {
        return -1;
    }
    Msgf(("ChnMoreMem(chn=%#lx, newsz=%ld, address=%#lx)\n",
          (unsigned long)id,
          (unsigned long)(nnew * sz + ChnHDRSZ),
