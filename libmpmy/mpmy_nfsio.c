...
int MPMY_Global_Open(const char *path, int flags, ...) {
    int fd;
...
int MPMY_Global_Write(int fd, const void *buf, unsigned int nbytes) {
    int ret;
...
int MPMY_Global_Write0(int fd, const void *buf, unsigned int nbytes) {
    int ret = 0;
    int allbytes;
    char *allbuf;
...
int MPMY_Global_Read(int fd, void *buf, unsigned int nbytes) {
    int ret;
    ...
    for (i = 0; i < procnum; i++) seeklen += sizes[i];
    ...
    for (i = 0; i < nproc; i++) seekptr += sizes[i];
