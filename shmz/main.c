...
int main(int argc, char *argv[]) {
    ...
    if (do_nsquared) {
        set_k(lambda);
        Ring(btab, sizeof(body), nobj, btab, sizeof(body), nobj, TBODYSZ, set_body, do_shmz);
    } else {
    ...
    pqsortsetup_order(&outputsort, output_btab, output_nobj, sizeof(outbody), 0.1, 1, Realloc_f);
