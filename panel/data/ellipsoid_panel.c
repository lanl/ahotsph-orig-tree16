...
/* Code to produce triangular panels on an ellipsoid, and to output all the
   necessary stuff for the panel tree code     */

/* we start with a discretization of the sphere. Then we deform it to produce
   an ellipsoid */
...
void CutTriangle(float pos0[3], float pos1[3], float pos2[3], int d, 
                 int l, int b[50], float aoR, float boR, float coR){
...
*/
      ...
      b[l]=0;
      CutTriangle(pos0, pos01, pos20, d, l, b, aoR, boR, coR);

      b[l]=1;
      CutTriangle(pos1, pos12, pos01, d, l, b, aoR, boR, coR);

      b[l]=2;
      CutTriangle(pos2, pos20, pos12, d, l, b, aoR, boR, coR);

      b[l]=3; 
      CutTriangle(pos01, pos12, pos20, d, l, b, aoR, boR, coR);
...
/* compute 4^d: */
    ...
    aoR=pow((aob*aob)*boc, .333333);
    boR=pow((boc*boc)*coa, .333333);
    coR=pow((coa*coa)*aob, .333333);

    fprintf(stderr, "a/R= %g\n", aoR);
    fprintf(stderr, "b/R= %g\n", boR);
    fprintf(stderr, "c/R= %g\n", coR);
...
First cut: an isocahedron:   */
    ...
    for(f=0; f<20; f++){
     ...
     CutTriangle(pos0, pos1, pos2, d, l, b, aoR, boR, coR);
