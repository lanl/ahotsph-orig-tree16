subroutine nsestart(t9fnl, rhofnl, yefnl, yp, yn, iident, iprocnum)
    parameter(nsetol = 1d - 4, kmax = 7) implicit double precision(a - h, o - z) T9start
    = 100.d0 rho = rhofnl ye = yefnl yp = ye yn = 1.d0 - ye ider = 0 T9 = T9start DelT9
    = dsign(20.d0, (t9fnl - t9start)) niter = 0 100 testt = t9fnl - t9 c write(*, *) 'testt',
             testt niter = niter + 1 If(niter.ge .1000) Then print *,
             'infinite loop in nsestart' call swerror Endif If(dabs(testt).gt.nsetol) Then call
...
c         write(*,999) t9fnl,t9,rho,ye,yp,yn,kit
          If(kit.ge.kmtmp) Then
             ...
             print*,'broken code',iprocnum,iident
             call swerror
c            stop
          Endif
      Endif
...
c           dgn2=0.d0
            ...
            DO 30 n=1,nmax
              ...
              znt=zn(n)
              zat=za(n)
              yt=dexp(max(dlgalp(n)+zpt*dlgyp+znt*dlgyn+h(iz), -80.d0))
...
c           dgp=dgp+dgp2
            ...
            ypt=yp-delyp
            ynt=yn-delyn
c Don't allow very small yp,yn (msw 4/9/2001)
            If(ypt.gt.1.d-75.and.ynt.gt.1.d-75) Then
                yp=ypt
                yn=ynt
            Else
                k=kmax
...
c--do loop in two parts for speed
          DO m=1,3
            ...
            zam=za(m)
            bim=bi(m)
            ym=dexp(max(dlgalp(m)+dlgyp*zpm+dlgyn*zn(m)+h(iz), -80.d0))
c           y(m)=ym
            xm=zam*ym
c           x(m)=xm
            zpym=zpm*ym
