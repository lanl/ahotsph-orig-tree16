...
void PollSetup(void put(void *buf, int size), int max_size, int tag) {
    ...
    if (max_size == sizeof(int) || max_size == 2 * sizeof(int))
        SinglError(
            "Poll uses size for message sorting.  You can't use size=%ld or %ld without some new "
            "coding\n",
            (long)sizeof(int),
            (long)2 * sizeof(int));
...
static void NLDone(int who, int tag) {
    ...
    while ((mask & done) && (mask < nproc)) {
        ...
        if (dest < nproc) { /* for non power-of-two sizes */
            ...
            MPMY_Isend(junk, sizeof(int), dest, tag, DllData(DllInsertAtBottom(&dlldone)));
        } else {
...
static void IBcast(const void *buf, int count, int tag) {
    ...
    for (chan = hibit(MPMY_Procnum()) + 1; chan < doc; chan++) {
        ...
        if (!(procnum & (1 << chan)) && sendproc < nproc) {
            ...
            MPMY_Isend(
                buf, count * sizeof(int), sendproc, tag, DllData(DllInsertAtBottom(&dlldone)));
