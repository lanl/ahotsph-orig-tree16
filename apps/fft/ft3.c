...
main(int argc, char *argv[])
{
	...
	for (j=0;j<nn[1];j++) {
	    for (k=0;k<nn[2];k++) {
		l = Index(i,j,k);
		data1[l].r=2*ran1(&idum)-1;
		data1[l].i=2*ran1(&idum)-1;
	    }
	}
    }
#else
    ...
    fp = MPMY_Fopen("ft3out", MPMY_WRONLY | MPMY_CREAT | MPMY_MULTI);

    MPMY_Fwrite(data1, nn[0]*nn[1]*nn[2]/nproc, sizeof(fcomplex), fp);
    MPMY_Fclose(fp);
