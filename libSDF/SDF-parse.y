...
char *SDFtype_names[] = {"notype", "char", "short", "int", 
				 "float", "double", "string"};

int SDFtype_sizes[] = {0, sizeof(char), sizeof(short), sizeof(int),
...
dcl1 : NAME {$$.name = $1; $$.type = curtype; $$.arrcnt = 1;}
    | NAME '[' CONST ']' 
	{
		...
		$$.name = $1; $$.type = curtype; $$.arrcnt = $3.u.intval;
	    }
	}
...
static int SDF_iomode = SDF_SYNC;
...
static int finish_parse(void)
{
    ...
    cur_hdr->datafp = MPMY_Fopen(Dataname, MPMY_RDONLY | MPMY_MULTI);
...
static int do_value_param(enum value_param_enum param, const_t value)
{
    switch(param){
    case BYTEORDER:
	if( value.type != SDF_INT )
	    return -1;
	cur_hdr->byteorder = value.u.intval;
...
#if YYDEBUG
    if(yydebug)
	printf("Declaration of %d records:\n", dcl.Nrec);
#endif
    if(no_more_data_blks){
