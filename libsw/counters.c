...
void ClearCounter(Counter_t *c) { c->counter = 0; }

void ClearEnabledCounters(void) {
    int i;
    for (i = 0; i < nenabled_counters; i++) { ClearCounter(enabled_counters[i]); }
}
...
void SumCounters(void) {
    ...
    for (i = 0; i < nenabled_counters; i++) {
        ...
        MPMY_ICombine(&c->min, &c->min, 1, MPMY_INT, MPMY_MIN, req);
        MPMY_ICombine(&c->max, &c->max, 1, MPMY_INT, MPMY_MAX, req);
...
void OutputCounters(int (*Printf_Like)(const char *, ...)) {
    ...
    SumCounters();
    Printf_Like("%12s %10s %10s %12s %12s\n", "Counters", "Min", "Max", "Sum", "Avg");
    for (i = 0; i < nenabled_counters; i++) {
        c = enabled_counters[i];
        if (c->enabled && c->name)
            Printf_Like("%12s %10d %10d %12.0f %12.2f\n", c->name, c->min, c->max, c->sum, c->mean);
    }
}
...
int ReadCounter(Counter_t *c) { return c->counter; }
