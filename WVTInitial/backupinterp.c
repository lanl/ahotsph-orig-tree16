...
int main(int argc, char *argv[]) {
    ...
    /*     if (keepcenterfixed && MPMY_Procnum() == 0) { */
    /*       btab[0].pos[0]=0.; */
    /*       btab[0].pos[1]=0.; */
    /*       btab[0].pos[2]=0.; */
    ...
    for (i = 1; i < nloop; ++i) {
        ...
        totvol = 4. / 3. * 3.1459 * outerbound * outerbound * outerbound;
        if (innerbound > 0)
            totvol -= 4. / 3. * 3.1459 * innerbound * innerbound * innerbound;
        ...
        SphericalGhosts(&btab, &nobj, outerbound, innerbound, &tothvol, totvol);
        SPHFindBbox(btab, nobj, rmin, rmax);
        sysradius = 0.5 * FixRsize(rmin, rmax);
    ...
    for (i = 1; i <= nhloop; i++) {
        ...
        for (p = btab; p < btab + nobj; p++) {
            if (p->ident < 10)
                singlPrintf("nbrs: %d oldh:%g ", p->nbrs, p->h);
            ...
            if (p->ident < 10)
                singlPrintf("newh: %g id:%d      ", p->h, p->ident);
        }
    }
