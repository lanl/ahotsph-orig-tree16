...
static Key_t phkey(unsigned long ikey[NDIM], unsigned int depth, int start, int type);
...
void FixId(body *btab, int nobj, int gnobj) {
    ...
    VerifyX(mynobj == nobj,
            Shout("mynobj=%d, nobj=%d, start=%d, gnobj=%d, nproc=%d, procnum=%d\n",
                  mynobj,
                  nobj,
                  start,
                  gnobj,
...
Key_t OutIdentKey(const outbody *bp) { return KeyLshift(KeyInt(bp->ident), KEYBITS / 2); }
#endif
