...
{
    SPHbody *p;
    double rho, u, udot, u2, ye, temp, abar, xp, xn, mass;
    double xpf, p2, p3, p4, temprev, rhoprev, xpprev, xnprev, yeprev,ufreez;
    double dtbrn,xhe4n,xc12n,xo16n,xne20n,xmg24n,xsi28n,xni56n;
    int ifleos;
	...
	xsi28n = p->xsi28n;
	xni56n = p->xni56n;
	if (xni56n > 0) Error("entering eos3 \n");
	if (xhe4n > 0) Error("entering eos3 \n");
	Fortran(eos3)(&steps, &mass, &rho, &u, &udot, &u2, &ye, &temp, 
		      ...
		      &xsi28n, &xni56n, &dtbrn);  
	if (xni56n > 0) Error("exiting eos3 \n");
	if (xhe4n > 0) Error("exiting eos3 \n");
