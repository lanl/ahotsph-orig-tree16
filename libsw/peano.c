...
static unsigned int _IntsFromPHKey(
    Key_t key, unsigned int ikey[NDIM], int depth, int start, int type) {
...
unsigned int IntsFromPHKey(Key_t key, unsigned int ikey[NDIM], int ndim) {
    assert(ndim == NDIM);
