...
//    if (params->timeout > 0) MPMY_TimeoutSet(params->timeout);
        ...
        if (strlen(params->SPHdatafile) > 0 || params->do_restart) {
            ...
            if (params->do_winds) {
                ...
                if (params->const_winds || params->nonconst_winds || params->accreting_winds) {
                    ...
                    if (params->const_winds || params->nonconst_winds)
                        SDFgetfloatOrDefault(
                            sdfp, "openangle_wind", &(params->openangle_wind), 180.0);
    ...
    SDFgetfloatOrDefault(
        sdfp, "dark_dt", &(params->dark_dt), (params->do_grav ? params->dt : 1e30));
...
if (SDFhasname ("outfile", sdfp))
    ...
    if (SDFhasname("kernel_ncoef1", sdfp)) {
        ...
        if (params->kernel_ncoef1 >= MAXCOEF)
            Error("Increase MAXCOEF\n");
        SDFgetintOrDie(sdfp, "kernel_ncoef2", &(params->kernel_ncoef2));
        if (params->kernel_ncoef2 >= MAXCOEF)
            Error("Increase MAXCOEF\n");
        if (SDFseekrdvecs(
                sdfp, "kernel_coef1", 0, params->kernel_ncoef1, &(params->kernel_coef1), 0, NULL))
            Error("SDFread kernel_coef1 failed\n");
        if (SDFseekrdvecs(
                sdfp, "kernel_coef2", 0, params->kernel_ncoef2, &(params->kernel_coef2), 0, NULL))
            Error("SDFread kernel_coef2 failed\n");
    } else {
        /* Monaghan spline kernel is default */
        params->kernel_ncoef1 = params->kernel_ncoef2 = 4;
        params->kernel_coef1[0] = 1.0;
        params->kernel_coef2[0] = 2.0;
        params->kernel_coef1[1] = 0.0;
        params->kernel_coef2[1] = -3.0;
        params->kernel_coef1[2] = -3.0 / 2.0;
        params->kernel_coef2[2] = 3.0 / 2.0;
        params->kernel_coef1[3] = 3.0 / 4.0;
        params->kernel_coef2[3] = -1.0 / 4.0;
    }
...
void print_initial_ctl(setup_params_t params) {
    ...
    for (int i = 0; i < params.kernel_ncoef1; i++) singlPrintf("%12.9f ", params.kernel_coef1[i]);
    singlPrintf("\n\t");
    for (int i = 0; i < params.kernel_ncoef2; i++) singlPrintf("%12.9f ", params.kernel_coef2[i]);
    singlPrintf("\n");
    if (params.log_time)
