...
MPMYFile *MPMY_Fopen(const char *path, int mpmy_flags) {
    ...
    Msgf(("Fopen %s\n", path));
    if (mpmy_flags & MPMY_RDONLY)
        flags |= O_RDONLY;
    if (mpmy_flags & MPMY_WRONLY)
        flags |= O_WRONLY;
    if (mpmy_flags & MPMY_RDWR)
        flags |= O_RDWR;
    if (mpmy_flags & MPMY_APPEND)
        flags |= O_APPEND;
    if (mpmy_flags & MPMY_TRUNC)
        flags |= O_TRUNC;
    if (mpmy_flags & MPMY_CREAT)
        flags |= O_CREAT;

    if (mpmy_flags & MPMY_MULTI)
        iomode = MPMY_MULTI;
    if (mpmy_flags & MPMY_SINGL)
        iomode = MPMY_SINGL;

    /* This is a sub-mode which can work with either of the above */
    if (mpmy_flags & MPMY_NFILE)
        iotype = MPMY_NFILE;
...
static int fseekrd_nfile(int fd, long offset, int whence, void *buf, int reclen, int nrecs) {
    int offend;
    ...
    for (i = 0; i < nproc; i++) {
        if (allbuf[i].start <= offset && offset < allbuf[i].start + allbuf[i].flen) {
            ...
            if (size > reclen * nrecs)
                size = reclen * nrecs;
            Msgf(("irecv %d from %d\n", size, i));
            if (nreq >= MAXREQ)
                Error("Too man Irecvs in fseekrd\n");
            ...
            if (nreq >= MAXREQ)
                Error("Too man Irecvs in fseekrd\n");
            MPMY_Irecv((char *)buf + reclen * nrecs - size, size, i, MPMY_IOTAG + nreq, &req[nreq]);
            ++nreq;
        }
    }
...
static int fseekrd(int fd, long offset, int whence, void *buf, int reclen, int nrecs) {
    int doseek;
    int real_whence;
    int nread = 0;
    int len;
