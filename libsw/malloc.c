...
size_t malloc_heapsz(void) { return 0; }
size_t malloc_used(void) { return 0; }
...
size_t malloc_heapsz(void) {
    void *curtop = sbrk(0);
    return ((long)curtop - (long)_chunk_tbl[0].begin);
}

size_t malloc_used(void) { return malloc_heapsz() - avail.size; }
...
/* of the block actually obtained. */
{
    ...
    if (sizeof(aheader_t) != sizeof(trailer_t))
        Error("sizeof(aheader_t) (%d) != sizeof(trailer_t) (%d)\n",
              (int)sizeof(aheader_t),
              (int)sizeof(trailer_t));
    ...
    if ((long int)begin == -1L) {
        Msgf(("sbrk(%ld) returns -1\n", (long)(n + sizeof(aheader_t) + sizeof(trailer_t))));
        if ((long int)(max_brk = getmaxbrk()) == -1L) {
            Warning("getmaxbrk() failed\n");
            return -1;
        }
        if ((long int)(begin = sbrk(0)) == -1L) {
            /* The OS is VERY confused and can't even tell us */
            /* where the top of memory is.  On Ncube this is because */
            /* the stack has grown to within 128 (I think) of the top */
            /* of memory since the last call to sbrk.  Sbrk doesn't want */
            /* to admit to the user that he can have memory this close to */
            /* the stack.  DUMB! */
            Warning("sbrk(0) failed\n");
            return -1;
        }
        if ((char *)max_brk < (char *)begin + sizeof(aheader_t) + sizeof(trailer_t)) {
            Warning("no more memory at all!\n");
            return -1;
        }
        n = ((char *)max_brk - (char *)begin) - (sizeof(aheader_t) + sizeof(trailer_t));
        Msgf(("Calling brk(%#lx)\n", (unsigned long)max_brk));
        if (brk(max_brk)) {
            /* Now there is some confusion in the OS.  We have chosen */
            /* max_brk so that it should, by construction, be a reasonable */
            /* value.  The OS has turned around and said it can't set */
            /* the top of memory to that value anyway.  There is probably */
            /* a problem with getmaxbrk */
            Warning("brk(%#lx) failed\n", (unsigned long)max_brk);
            return -1;
        }
    }
...
int malloc_verify(void) {
    ...
    /* information */
    if ((_nchunks != MAXCHUNKS) && (sz_free != avail.size || n_free_blocks != n_free_blocks1)) {
        sprintf(malloc_errstring,
                "Sizes don't add up, sz_free=%ld, avail.size=%ld, n_free_blocks=%d, "
                "n_free_blocks1=%d!\n",
                (long)sz_free,
                (long)avail.size,
                n_free_blocks,
...
static int verify_blk(fheader_t *p) {
    ...
    if ((p->tag != FREE && p->tag != INUSE)) {
        sprintf(malloc_errstring, "Bad magic byte in hdr %#lx\n", (unsigned long)p);
        return 1;
    }
    ...
    if (p->tag != tp->tag) {
        sprintf(
            malloc_errstring, "Tags don't match %#lx, %#lx\n", (unsigned long)p, (unsigned long)tp);
        return 1;
    }
    ...
    /* such a block */
    if (p->size < sizeof(fheader_t) + sizeof(trailer_t) && p->size != sizeof(trailer_t)) {
    ...
    if (p->tag == FREE) {
        ...
        if (l != &avail && (l >= top_of_mem || l < bottom_of_mem)) {
            sprintf(malloc_errstring,
                    "Link ptr (%#lx) of %#lx out of range\n",
                    (unsigned long)l,
                    (unsigned long)p);
            return 1;
        }
        ...
        if (l != &avail && (l >= top_of_mem || l < bottom_of_mem)) {
            sprintf(malloc_errstring,
                    "Prev ptr (%#lx) of %#lx out of range\n",
                    (unsigned long)l,
                    (unsigned long)p);
            return 1;
        }
...
#include <ulimit.h>
static void *getmaxbrk(void) { (void *)ulimit(UL_GETMAXBRK); }
#endif

#if defined(sun) || defined(__PARAGON__) || defined(__iX86__)
...
static void *getmaxbrk(void) { return (void *)(BEGIN + 12 * 1024 * 1024); /* 12Meg */ }
#endif

#ifndef GETMAXBRK_DEFINED
static void *getmaxbrk(void) { return (void *)-1; }
