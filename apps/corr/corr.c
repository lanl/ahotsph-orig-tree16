...
typedef struct {
    ...
    float pos[NDIM];		/* position of body */
    float vel[NDIM];		/* velocity of body */
    float mass_hist[HIST_BINS];
    int   nobj_hist[HIST_BINS];
    float rvel_hist[HIST_BINS];
    float rvel2_hist[HIST_BINS];
    float vel2_hist[HIST_BINS];
} cbody;
...
do_corr(void *p0, void *list, int bsize, int n)
{
    ...
    const float maxr2 = Max_radius2*(1.0-1e-5);	/* avoid roundoff errors */
...
main(int argc, char *argv[])
{
    ...
    float mass_corr[HIST_BINS];
    int nobj_corr[HIST_BINS];
    float nobj_corrf[HIST_BINS];
    float rvel_corr[HIST_BINS];
    float rvel2_corr[HIST_BINS];
    float vel2_corr[HIST_BINS];
	...
	for (j = 0; j < HIST_BINS; j++) {
	    ctab[cnobj].mass_hist[j] = (float)0.0;
	    ctab[cnobj].nobj_hist[j] = 0;
	    ctab[cnobj].rvel_hist[j] = (float)0.0;
	    ctab[cnobj].rvel2_hist[j] = (float)0.0;
	    ctab[cnobj].vel2_hist[j] = (float)0.0;
	}
    ...
    for (i = 0; i < cnobj; i++) {
	for (j = 0; j < HIST_BINS; j++) {
	    mass_corr[j] += ctab[i].mass_hist[j];
	    nobj_corr[j] += ctab[i].nobj_hist[j];
	    rvel_corr[j] += ctab[i].rvel_hist[j];
	    rvel2_corr[j] += ctab[i].rvel2_hist[j];
	    vel2_corr[j] += ctab[i].vel2_hist[j];
	}
    }
    MPMY_Combine(&total_mass, &total_mass, 1, MPMY_FLOAT, MPMY_SUM);

    for (i = 0; i < HIST_BINS; i++) {
	nobj_corrf[i] = nobj_corr[i];
	mean_radius[i] = exp(0.5*(i+0.5)/LogFactor) * win_min;
	radius[i] = exp(0.5*(i+1.0)/LogFactor) * win_min;
	massb[i] = (4.0*M_PI/3.0) * (total_mass/physical_vol)
	  * radius[i] * radius[i] * radius[i];
	nobjb[i] = (4.0*M_PI/3.0) * (gnobj/physical_vol)
	  * radius[i] * radius[i] * radius[i];
    }

    MPMY_Combine(mass_corr, mass_corr, HIST_BINS, MPMY_FLOAT, MPMY_SUM);
    MPMY_Combine(nobj_corr, nobj_corr, HIST_BINS, MPMY_INT, MPMY_SUM);
    MPMY_Combine(nobj_corrf, nobj_corrf, HIST_BINS, MPMY_FLOAT, MPMY_SUM);
    MPMY_Combine(rvel_corr, rvel_corr, HIST_BINS, MPMY_FLOAT, MPMY_SUM);
    MPMY_Combine(rvel2_corr, rvel2_corr, HIST_BINS, MPMY_FLOAT, MPMY_SUM);
    MPMY_Combine(vel2_corr, vel2_corr, HIST_BINS, MPMY_FLOAT, MPMY_SUM);

    rvel_corr[i] /= (nobj_corrf[0]+1e-6);
    rvel2_corr[i] /= (nobj_corrf[0]+1e-6);
    vel2_corr[i] /= (nobj_corrf[0]+1e-6);
    mass_corr[0] /= nsamples*massb[0];
    nobj_corrf[0] /= nsamples*nobjb[0];
    for (i = 1; i < HIST_BINS; i++) {
	rvel_corr[i] /= (nobj_corrf[i]+1e-6);
	rvel2_corr[i] /= (nobj_corrf[i]+1e-6);
	vel2_corr[i] /= (nobj_corrf[i]+1e-6);
	mass_corr[i] /= nsamples*(massb[i]-massb[i-1]);
	nobj_corrf[i] /= nsamples*(nobjb[i]-nobjb[i-1]);
    }

    singlPrintf("Writing to %s\n", outname);
	...
	fprintf(fp, "# input datfile %s\n", name);
	fprintf(fp, "# Mass variable is = %s\n", mass_name);
	fprintf(fp, "# float win_min = %g\n", win_min*(1.0+redshift));
	fprintf(fp, "# float win_max = %g\n", win_max*(1.0+redshift));
	fprintf(fp, "# float physical_l0 = %g\n", physical_l0);
	fprintf(fp, "# float redshift = %g\n", redshift);
	fprintf(fp, "# float hubble = %g\n", hubble);
	fprintf(fp, "# float total_mass = %g\n", total_mass);
	fprintf(fp, "# int gnobj = %d\n", gnobj);
	fprintf(fp, "# int nsamples = %d\n", nsamples);
	fprintf(fp, "# values for radius and velocities are comoving (1+z), sqrt(1+z)\n");
	fprintf(fp, "# bin_num radius nobj_corr mass_corr v_rad v_rad_rms v_rms n\n");
	for (j = 1; j < HIST_BINS; j++) {
	    fprintf(fp, "%2d %10g %10g %10g %10g %10g %10g %d\n", 
		    j, mean_radius[j]*(1.0+redshift), nobj_corrf[j], 
		    mass_corr[j], rvel_corr[j]/(1.0+redshift), 
		    sqrt(rvel2_corr[j])/sqrt(1.0+redshift), 
		    sqrt(vel2_corr[j])/sqrt(1.0+redshift), 
		    nobj_corr[j]);
	}
	Fclose(fp);
    }
