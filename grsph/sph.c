...
void SetSPH(
    float visc_alpha, float visc_beta, float eos_gamma, int gnobj, void bfunc(), void cfunc()) {
    Nobj = gnobj;
    alpha = visc_alpha;
    Gamma = eos_gamma;
    beta = visc_beta;
...
void update_intermediate(body *btab, int nobj, float dt_last, int flag) {
    ...
    for (p = btab; p < btab + nobj; p++) {
        ...
        c3 = (float)1.0 + ux * ux * p->guxx + uy * uy * p->guyy + uz * uz * p->guzz
             + 2.0 * (ux * uy * p->guxy + ux * uz * p->guxz + uy * uz * p->guyz);
        ut = (-c2 + sqrtf_fast(c2 * c2 - c1 * c3)) / c1;

        uut = ut * p->gutt + ux * p->guxt + uy * p->guyt + uz * p->guzt;
        vx = ut * p->guxt + ux * p->guxx + uy * p->guxy + uz * p->guxz;
        vy = ut * p->guyt + ux * p->guxy + uy * p->guyy + uz * p->guyz;
        vz = ut * p->guzt + ux * p->guxz + uy * p->guyz + uz * p->guzz;
        ...
        p->vsound = sqrtf_fast(p->u * Gamma * (Gamma - (float)1.0) / (p->u * Gamma + (float)1.0));

        gamadot = (p->gama - p->gama_last) / dt_last;
        p->udot = -p->pr * gamadot / p->rho_est
                  + p->drho_dt * p->pr * p->gama / (p->rho_est * p->rho_est);
