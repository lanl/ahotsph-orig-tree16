...
#if defined(USE_ALARM) && !(defined(__iX86__) || defined(_AIX) || defined(__SUN5__))
#define HAVE_SIGVEC
#endif
...
int select(
    int width, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout);
...
void Sinit_elt(void) {
    ...
    assert(getenv("LSV_PROCNUM") && getenv("LSV_NPROC") && getenv("LSV_HOSTPORT")
           && getenv("LSV_HOST"));
...
void Sinit_host(int n) /* n is how many nodes we talk to */
{
    int i, nin;
    int size = sizeof(struct sockaddr_in);
...
int Srecv_block(void *inb, int size, int type, int *from) {
    ...
    tryagain:
        ...
        if (sent < 0) {
            if (sent == BRECV_TIMEDOUT) {
                Warning(
                    "Srecv_block: brecv(size=%d, from=%d, type=%d, timeout=%d) timed out, "
                    "inbytes=%d, errno=%d\n",
                    size,
                    *from,
                    type,
...
void Sdiag(int (*printf_like)(const char *, ...)) {
    ...
    int *buf;
    printf_like(
        "lsv.c:  counters to and from various destinations\n[dest seqout[dest] seqin[dest] "
        "nretry[dest]]\n");
...
ackrecv:
    ...
    if (intype != ACK_TYPE) { /* got a data packet */
        if (seq < seqin[src])
            ...
            msgcnt++;
            if (msgcnt >= MAXDEFER)
                Error("msgcnt too large\n");
            Msgf(("adefer seq:%d type:%d, len:%d from %d ", seq, intype, inlen, src));
        }
...
datrecv:
    ...
    if (intype == ACK_TYPE) {
        ...
        goto datrecv;
    } else if (type == intype && seq == seqin[src] && (*dest == src || *dest == LSV_ANY)) {
        if (*dest == LSV_ANY) {
            *dest = src;
...
static void send_ack(int s, int dest, int seq, struct sockaddr_in *dest_addr) {
    ...
    ack[3] = Swap(ACK_TYPE);
    ack[4] = Swap(0);
    if (sendto(s, (void *)ack, HLEN, 0, (struct sockaddr *)dest_addr, sizeof(struct sockaddr_in))
        != HLEN)
