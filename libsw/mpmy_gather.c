...
int MPMY_AllGather(const void *sendbuf, int count, MPMY_Datatype type, void *recvbuf) {
    int chan;
    int doc;
    MPMY_Status stat;
    int sendproc;
...
int MPMY_Gather(const void *sendbuf, int count, MPMY_Datatype type, void *recvbuf, int recvproc) {
    ...
    if (recvproc != 0)
        Error("Gather to procnum != 0 not supported yet.\n");
...
int MPMY_NGather(
    const void *sendbuf, int count, MPMY_Datatype type, void **recvhndl, int recvproc) {
    int chan;
    int doc;
    int sendproc;
    long bufsz;
    ...
    if (recvproc != 0)
        Error("NGather to procnum != 0 not supported yet.\n");
...
int MPMY_BcastTag(void *buf, int count, MPMY_Datatype type, int srcproc, int tag) {
    int chan;
    int doc;
    int sendproc;
    int procnum = MPMY_Procnum();
    ...
    if (srcproc != 0)
        Error("Bcast from procnum != 0 not supported yet.\n");

    Msgf(("MPMYBcast(buf=%p, count=%d, type=%d, srcproc=%d\n", buf, count, type, srcproc));
    Msg_flush();
    doc = ilog2(nproc);
    if (nproc != 1 << doc)
        doc++; /* for non power-of-two sizes */
