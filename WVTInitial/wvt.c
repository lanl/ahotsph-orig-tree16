...
void WVTInitCube(SPHbody **btabp,
    ...
    *gnobj = ((int)ceil((max[0] - min[0]) / delta[0])) * ((int)ceil((max[1] - min[1]) / delta[1]))
    ...
    for (pos[0] = min[0]; pos[0] <= max[0] + delta[0] / 3.0; pos[0] += delta[0]) {
        for (pos[1] = min[1]; pos[1] <= max[1] + delta[1] / 3.0; pos[1] += delta[1]) {
            for (pos[2] = min[2]; pos[2] <= max[2] + delta[2] / 3.0; pos[2] += delta[2]) {
...
void WVTInitHex(SPHbody **btabp,
    ...
    *gnobj = ((int)ceil((max[0] - min[0]) / delta[0])) * ((int)ceil((max[1] - min[1]) / delta[1]))
    ...
    for (pos[2] = zmin; pos[2] <= max[2]; pos[2] += delta[2]) {
        yoff = ((int)round(fabs(pos[2]) / delta[2]) % 2) * delta[1] * (1.0 - 1.0 / sqrt(3.0));
        ymin = min[1] + yoff;
...
void WVTInitHex2(SPHbody **btabp,
                 int *gnobj,
                 int *nobj,
                 double min[NDIM],
                 double max[NDIM],
    ...
    *gnobj = ((int)ceil((max[0] - min[0]) / dx)) * ((int)ceil((max[1] - min[1]) / dy))
...
void WVTInitProbdist(SPHbody **btabp,
    ...
    *gnobj = targetnobj;
    ...
    i = 0;
    for (pos[0] = min[0] + delta[0] / 2.; pos[0] <= max[0]; pos[0] += delta[0]) {
        for (pos[1] = min[1] + delta[1] / 2.; pos[1] <= max[1]; pos[1] += delta[1]) {
            for (pos[2] = min[2] + delta[2] / 2.; pos[2] <= max[2]; pos[2] += delta[2]) {
        ...
        for (pos[1] = min[1] + delta[1] / 2.; pos[1] <= max[1]; pos[1] += delta[1]) {
            for (pos[2] = min[2] + delta[2] / 2.; pos[2] <= max[2]; pos[2] += delta[2]) {
                ...
                if (r < outerbound && r > innerbound) {
                    ...
                    if (subtotrho / totrho >= (double)MPMY_Procnum() / ((double)MPMY_Nproc())
                        && subtotrho / totrho
    ...
    *gnobj = targetnobj;
    ...
    for (i = 0; i < num[0]; ++i) {
        ...
        if (r < outerbound && r > innerbound) {
            ...
            totrho
                += rho * ((r2 * r2 * r2 - r1 * r1 * r1) / (outerbound * outerbound * outerbound));
        }
    }
    ...
    for (i = 0; i < num[0]; ++i) {
        ...
        if (r < outerbound && r > innerbound) {
            ...
            subtotrho
                += rho * ((r2 * r2 * r2 - r1 * r1 * r1) / (outerbound * outerbound * outerbound));
            rho *= ((double)*gnobj) / totrho
                   * ((r2 * r2 * r2 - r1 * r1 * r1) / (outerbound * outerbound * outerbound));
            nadded = ignpoi((float)rho);

            if ((subtotrho / totrho >= (double)MPMY_Procnum() / ((double)MPMY_Nproc())
                 && subtotrho / totrho < ((double)MPMY_Procnum() + 1) / ((double)MPMY_Nproc()))
                || (i == num[0] - 1) && (MPMY_Procnum() + 1 == MPMY_Nproc())) {
                for (j = 0; j < nadded; j++) {
...
void WVTInitProbdist_lotsmem(SPHbody **btabp,
                             int *gnobj,
                             int *nobj,
                             double min[NDIM],
                             double max[NDIM],
    ...
    *gnobj = targetnobj;
    ...
    for (p = q; p < q + nbox; p++) {
        if (p->mass / totrho >= (double)MPMY_Procnum() / ((double)MPMY_Nproc())
            && p->mass / totrho < ((double)MPMY_Procnum() + 1) / ((double)MPMY_Nproc())) {
...
void WVTInitCube2(SPHbody **btabp,
                  int *gnobj,
                  int *nobj,
                  double min[NDIM],
                  double max[NDIM],
...
void WVTInitProbdist2(SPHbody **btabp,
                      int *gnobj,
                      int *nobj,
                      double min[NDIM],
                      double max[NDIM],
    ...
    *gnobj = targetnobj;
    ...
    for (p = q; p < q + nbox; p++) {
        ...
        /* 	    r=1;  */
        if (testran < rho && r < outerbound && r > innerbound) {
            qq = StkPush(&ss, sizeof(SPHbody));
...
void macWVT(SinkSPH *sink, hcell **source_vec, int *result, int n) {
    ...
    double q = 2.;     /* Should definitely be >= 2! Extent to which neighbors are
                          enclosed */
    double res1 = sink->udot, res2 = sink->rho_est, res3 = sink->vsound, res4 = sink->temp;
...
void WVT_setinputoption(int option) { inputoption = option; }
...
void WVT_hofpos_pwl(SPHbody *btab, int nobj, double totvol, double *tothvol, int dim) {
    ...
    for (p = btab; p < btab + nobj; p++) { p->h *= q * pow((totvol / *tothvol), 0.333333333); }
}
...
void macConstNeigh(SinkSPH *sink, hcell **source_vec, int *result, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((rij = sqrtf_fast(dr2)) > extent_sink + extent_src || dr2 == (double)0.0) {
            goto accept;
        } else if (daughters != 1) {
            goto failed;
        }
...
void WVT_hofpos_inputh(SPHbody *btab, int nobj, double totvol, double *tothvol, int dim) {
    ...
    if (nobj == -1) {
        ...
        for (p = btab; p < btab + nobj; p++) {
            ...
            p->h = hglob[index]
                   + (rad - rglob[index]) / (rglob[index + 1] - rglob[index])
        ...
        for (p = btab; p < btab + nobj; p++) { p->h *= q * pow((totvol / *tothvol), 1. / dim); }
    }
}
...
void WVT_hofpos_cylgrid(SPHbody *btab, int nobj, double totvol, double *tothvol, int dim) {
    ...
    if (nobj < 0)
        singlPrintf("Using Cylindrical Grid data.\n");
    ...
    interp_cylindricalgrid(
        btab, nobj, dimr, dimz, dimtheta, minr, maxr, minz, maxz, mintheta, maxtheta, bgrho);


    if (redo_tothvol) {
        for (p = btab; p < btab + nobj; p++) { *tothvol += p->h * p->h * p->h; }
        *tothvol *= 4. * sqrt(3.);
        MPMY_Combine(tothvol, tothvol, 1, MPMY_DOUBLE, MPMY_SUM);
    }
...
void interp_cylindricalgrid(SPHbody *SPHbtab,
    ...
    if (nobj != -1) {
        for (q = SPHbtab; q < SPHbtab + nobj; q++) {
            ...
            r = sqrt(pow(q->pos[0] + cylcenter[0], 2) + pow(q->pos[1] + cylcenter[1], 2));
                ...
                for (tk = 0; tk < 2; tk++) {
                    for (tj = 0; tj < 2; tj++) {
                        if (k + tk >= dimz || j + tj >= dimr) {
                            ...
                            w = (double)1.
                                / (pow(ll - tl, 2) + pow(kk - tk, 2) + pow(jj - tj, 2) + 1e-10);
                            sumprop += w * cylgrid[j + tj][k + tk][(l + tl) % dimtheta];
                            sumprop_h += w * cylgrid_h[j + tj][k + tk][(l + tl) % dimtheta];
                            sumw += w;
                        }
                    }
                }
...
void init_cartesiangrid(int tdimx,
                        int tdimy,
                        int tdimz,
                        double tminx,
                        double tmaxx,
    ...
    if (nobj != -1) {
        for (q = SPHbtab; q < SPHbtab + nobj; q++) {
            ...
            for (tl = 0; tl < 2; tl++) {
                for (tk = 0; tk < 2; tk++) {
                    for (tj = 0; tj < 2; tj++) {
                        if (j + tj < 0 || j + tj >= dimx || k + tk < 0 || k + tk >= dimy
                            || l + tl < 0 || l + tl >= dimz) {
                            w = 0.;
                        } else {
                            /* weight is the inverse squared of the distance */
...
void WVT_hofpos_cartgrid(SPHbody *btab, int nobj, double totvol, double *tothvol, int dim) {
    ...
    if (nobj < 0)
        singlPrintf("Using Cartesian Grid data.\n");
