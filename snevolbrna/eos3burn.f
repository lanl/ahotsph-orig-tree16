...
c
     ...
     $     xo16ni,xne20ni,xmg24ni,xsi28ni,xni56ni,dtbrni)
      c *************************************************************c c compute
          ...
          xheavyi real dtburn double precision dtbrni, uburni, pmassi, dui, $ xhe4ni, xc12ni,
          xo16ni, xne20ni, xmg24ni, xsi28ni, xni56ni c common / output / vsoundi, pri, etai, yehi,
          xmuei, xmuhati, xalphai, $ xheavyi c common / konst / gg, clight, arad, bigr, xsecnn,
          ...
          yei stop endif print *,
          'inside eos3' c c-- assume chemical freeze - out c ui
          = max(1.e-5, ui) call eosfl(rhoi,
                                      pri,
                                      $ ui,
                                      u2i,
                                      yei,
                                      ...
                                      $ yeprev,
                                      xpprev,
                                      xnprev) if (ifleosi.eq .1) then stot
          = u2i call rootemp2u(rhoi, ui, tempi, yei, abari, 1 ptot, cs, etai, stot) xpi = 0. xni
          = 0. xmuei = etai *tempi xmuhati = 0.0 xalphai = 0.0 xheavyi = 1. yehi
          = yei c c-- ocean eos + burning c elseif(ifleosi.eq .2) then abari
          = 1d0
            / (xhe4ni / 4.d0 + xc12ni / 12.d0 + $ xo16ni / 16.d0 + $ xne20ni / 20.d0
               + xmg24ni / 24.d0 + $ xsi28ni / 28.d0 + xni56ni / 56.d0 + $ xpi + xni)
                call rootemp2u(rhoi, ui, tempi, yei, abari, $ ptot, cs, etai, stot) xmuei
          ...
          = u2i end if call rootemp4(rhoi,
                                     ...
                                     stot) c-- assume all dissociated,
                                                                                    ...
                                                                                    = xni xpprev
                                                                                    = xpi yeprev
                                                                                    = yei c c-- call burning network c
                                                                                        c call burner(
                                                                                            dtburn,
                                                                                            dtbrni,
                                                                                            tempi,
                                                                                            ...
                                                                                            iexpl)
      c c ui = ui
               + uburni c return end c subroutine rootemp2s(
                   rhoi, ui, tempi, yei, 1 abar, ptot, cs, eta, si)
c*****************************************************************
c                                                                *
...
c**************************************************************** c c this subroutine determines
    ...
    tfreeze c rhoswe = 1.05d3 c rhoswe = 2.05d3 t9nse = 21.d0 t9burn = 4. tfreeze
    = t9nse - 7.0d0 c c-- entropy conversion factor sfac = avokb* utemp / uergg c rhocgs
    = rhoi
      * udens if (ifleosi.eq .1.and.(tempi.gt.t9nse.or.rhoi.gt .4.0 * rhoswe)) then call
...
call nsetemp(
    ...
    = tempi endif c return end c subroutine burner(dtburn,
                                                   dtbrni,
                                                   tempi,
                                                   rhoi,
                                                   ui,
...
c--return here if failed
 10      continue
         ...
         xn1=xn1_old
         abari=abar_old
         if (tempin.gt.dble(t9nse)) goto 200
...
c--dtemp varies more than 3%
         if (isucc.eq.-1.or.dxmx.gt.0.05d0.or.dtemp.gt.0.03d0) then
...
c call snuc(rdpg,
            ...
            r1216,
            r1616)
c xc12 = xn(1) xo16 = xn(2) xne20 = xn(3) xmg24 = xn(4) xsi28 = xn(5) xni56 = xn(6) xhe4 = xn(7) xn1
    = xn(8) xp1 = xn(9) c xsum
    = 1.d0
      - (xp1 + xn1 + xhe4 + xc12 + xo16 + xne20 + xmg24 + xsi28 + xni56) if (dabs(xsum).lt.xoff)
          then isucc
    = 1 elseif(dabs(xsum).lt.(10.0d0 * xoff)) then isucc = 0 c-- reset largest abundance xmx
    = dmax1(xn1, xp1, xhe4, xc12, xo16, xne20, xmg24, xsi28, xni56) if (xo16.ge.xmx) then xo16
    = xo16 + xsum elseif(xsi28.ge.xmx) then xsi28 = xsi28 + xsum elseif(xni56.ge.xmx) then xni56
    = xni56 + xsum elseif(xmg24.ge.xmx) then xmg24 = xmg24 + xsum elseif(xne20.ge.xmx) then xne20
    = xne20 + xsum elseif(xhe4.ge.xmx) then xhe4 = xhe4 + xsum elseif(xc12.ge.xmx) then xc12
    = xc12 + xsum elseif(xn1.ge.xmx) then xn1 = xn1 + xsum else xp1 = xp1 + xsum endif else isucc
    = -1 endif dubdtcgs = energy c return end c

        subroutine rate(rdpg,
                        rhegp,
                        r3a,
                        rg3a,
                        rcag,
...
c.. passed in common /scrn/ to subr rate
      ...
      subroutine leqs(n)
      implicit real * 8(a - h, o - z) c c....leqs performs matrix inversion c c n
...
c      if (t9.le.3.0) mdim= 7
      call leqs(mdim)
      c c....calculate energy generation rate c enuc
          = 0. do 50 i = 1,
                     9 50 enuc = enuc + be(i) * bb(i) enuc
                     = enuc * 9.647e+17 / dt c c....calculate mass fractions c do 60 i = 1,
                     9 60 xn(i) = xn(i) + an(i) * bb(i)
      c return end
