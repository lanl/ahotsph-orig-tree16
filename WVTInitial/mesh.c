...
double MeshGetCost(const Meshbody *ptr) { return 1.0; }


Key_t MeshGetKeyFromStruct(const Meshbody *ptr) { return ptr->key; }

p void MeshCofmFromDaugh(hcellptr hptr, hcellptr daughters[])
...
void Meshgate(Meshbody *sink, hcell **source_vec, int *result, int n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (wtij < (double)0.0)
            Error("Negative wtij (macRho) = %g %g\n", wtij, hmean11);
