...
bool check_all(const state_t* state, const int n) {
    ...
    double times[nlines], strain[nlines], strain_rate[nlines], stress[nlines], temp[nlines],
        G[nlines], rho[nlines];
    ...
    for (int idx = 0; idx < nlines; ++idx) {
        if (!approx_equal(&(state->time[idx + 1]), &(times[idx]), &rel_err))
            printf(
                "Error: ptw test failed at iteration %i: time not equal. Expected: %f, actual: "
                "%f\n",
                idx,
                times[idx],
                state->time[idx + 1]);
        if (!approx_equal(&(state->stress[idx + 1]), &(stress[idx]), &rel_err))
            printf(
                "Error: ptw test failed at iteration %i: stress not equal. Expected: %f, actual: "
                "%f\n",
                idx,
                stress[idx],
                state->stress[idx + 1]);
        if (!approx_equal(&(state->temp[idx + 1]), &(temp[idx]), &rel_err))
            printf(
                "Error: ptw test failed at iteration %i: temperature not equal. Expected: %f, "
                "actual: %f\n",
                idx,
                temp[idx],
                state->temp[idx + 1]);
        if (!approx_equal(&(state->G[idx + 1]), &(G[idx]), &rel_err))
            printf(
                "Error: ptw test failed at iteration %i: G not equal. Expected: %f, actual: %f\n",
                idx,
                G[idx],
                state->G[idx + 1]);
    }
...
bool check_update_T(const double* stress,
    ...
    temp_calc
        = temp_test + (double)cond * consts->chi * (*stress) * (*edot) * (*dt) / ((*C_v) * (*rho));
