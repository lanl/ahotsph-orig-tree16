...
void ABMSetup(ABM *abm, int pktsize, int tag, int nfuncs, ABMhndlr_t *hndlarray[]) {
    ...
    abm->tag = tag;
    Ver(MPMY_Irecv(abm->recvbufA, abm->pktsize, MPMY_SOURCE_ANY, abm->tag, &abm->Recv_Hndl));
    ...
    for (i = 0; i < nproc; i++) { DllCreate(&abm->Enqueued[i], &abm->QelmtChn); }
...
int ABMAllDone(ABM *abm) { return abm->alldone; }
...
void ABMShutdown(ABM *abm) {
    ...
    assert(MPMY_Source(&stat) == MPMY_Procnum() && MPMY_Count(&stat) == sizeof(int));

    for (i = 0; i < MPMY_Nproc(); i++) { DllTerminate(&abm->Enqueued[i]); }
...
static void Donehndlr(ABM *abm, int who) {
    int mask, dest, type;
...
static void AllDonehndlr(ABM *abm) {
    int chan;
    int procnum = MPMY_Procnum();
...
int ABMPoll(ABM *abm) {
    ...
    /* We need to call Flick occasionally, and here seems as good a place
       as any. */
    MPMY_Flick();
...
static int ABMPoll_common(ABM *abm, MPMY_Status *status) {
    ...
    do {
        ...
        if (MPMY_Tag(status) != abm->tag)
            Error("Bad tag (%d) in ABMPoll(), should be %d\n", MPMY_Tag(status), abm->tag);
        src = MPMY_Source(status);
        len = MPMY_Count(status);
        Msgf(("ABMPoll Received %d byte packet from %d\n", len, src));

        in = abm->recvbufA;
        Ver(MPMY_Irecv(abm->recvbufB, abm->pktsize, MPMY_SOURCE_ANY, abm->tag, &abm->Recv_Hndl));
        ...
        while (in < end) {
            ...
            switch (type) {
                case ABMALLDONETYPE:
                    Msgf(("ABMALLDONE recvd from %3d.\n", src));
                    AllDonehndlr(abm);
                    break;
                case ABMDONETYPE:
                    Msgf(("ABMDONE recvd from %3d.\n", src));
                    Donehndlr(abm, src);
                    break;
                default:
                    Msgf(("ABMPoll type %d\n", type));
...
static int cmp_xor(const void *p1, const void *p2) {
    int d1 = *(int *)p1;
    int d2 = *(int *)p2;
    int procnum = MPMY_Procnum();
...
static void ABMFlushOne(ABM *abm, int dest) {
    ...
    for (q = DllTop(Q); q != DllInf(Q); q = DllDeleteDown(Q, q)) {
        ...
        if (Qelmt->sz + 2 * sizeof(int) > szleft) {
            ...
            Msgf(("SendQ full packet for %d (len=%d)\n", dest, used));
    ...
    used = abm->pktsize - szleft;
    Msgf(("SendQ: Remaining packet for %d (len=%d)\n", dest, used));
    SendPkt(abm, buf, used, dest);
    abm->cntarr[dest] = 0;
}
...
void ABMFlush(ABM *abm) {
    ...
    if (abm->ndests == 0)
        return; /* Required to avoid huge overhead. */
    ...
    for (i = 0; i < abm->ndests; i++) { ABMFlushOne(abm, abm->destarr[i]); }
    abm->ndests = 0;
    DeliveryTest(abm);
}
...
static void DeliveryTest(ABM *abm) {
    ...
    MPMY_Flick();
    for (pp = DllBottom(&abm->undeliveredLL); pp != DllSup(&abm->undeliveredLL);
...
static void DeliveryWait(ABM *abm) {
    Msgf(("ABMWait\n"));
    while (DllLength(&abm->undeliveredLL)) { DeliveryTest(abm); }
}
