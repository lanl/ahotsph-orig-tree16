...
static int MPI_IreduceInit(MPI_Request *reqp) {
    StkInitEz(&cstk);
    StkInitEz(&outdata);
    setup = 1;
    mask = ~0;
...
static int MPI_Ireduce(void *sendbuf,
                       void *recvbuf,
                       int count,
                       MPI_Datatype datatype,
                       MPI_Op op,
...
#define Do_Op(outbuf, op, inbuf, type, cnt)                      \
    do {                                                         \
        char *oend = (char *)outbuf + ALIGN(cnt * sizeof(type)); \
...
static void do_combine(void *inbuf, void *outbuf, int n, struct comb_st *manifest) {
    int i;
    int count;
...
int MPI_IreduceWait(MPI_Request req, MPI_Comm comm) {
    int chan;
    int doc;
    int i;
    void *inbuf;
