...
void check_sort(char *place, Tag_t *tag, int n) {
    int i;
    char msg[256];
...
static void merge_t(Tag_t *a, Tag_t *b, int na, int nb, Tag_t *t) {
    Tag_t *alast, *blast;
    alast = a + na;
    blast = b + nb;

#if !defined(__PARAGON__)
    while (a < alast && b < blast) *t++ = (KeyLT(a->key, b->key)) ? *a++ : *b++;
#else
...
static Tag_t *rsort_t(Tag_t *tag, int n, int radixBits) {
    int i, shift, nk;
    unsigned int *keyden;
    _KTYPE *k;
    Tag_t *tag2, *ktmp;
    ...
    for (nk = 0; nk < NK; nk++) {
        for (shift = 0; shift < KEYBITS / NK; shift += radixBits) {
            for (i = 0; i < radix; i++) keyden[i] = 0;
            for (k = &tag[0].key.k[nk]; k < &tag[n].key.k[nk]; k += stride)
                ++keyden[*k >> shift & mask];
            for (i = 1; i < radix; i++) keyden[i] += keyden[i - 1];
...
int main(int argc, char *argv[]) {
    ...
    check[1] = tagtbl[n - 1];
    if (procnum == 0)
        checktbl = Malloc(2 * nproc * sizeof(Tag_t));
    MPMY_Gather(check, sizeof(Tag_t) * 2, MPMY_CHAR, checktbl, 0);

    check_sort("final", tagtbl, n);
    if (procnum == 0 && nproc != 1)
        check_sort("global", checktbl, nproc * 2);
    singlPrintf("Passed.\n");
...
void pdrmsort(void *p, int n, int radixBits) {
    int i, j;
    Tag_t *tagtbl2;
    Tag_t *k, *l;
    int nproc, procnum;
    ...
    if (n < NPROBES)
        Error("n less than NPROBES\n");
    for (weight = 0.0, i = 0; i < n; i++) weight += tagtbl[i].weight;
    ...
    if (procnum == 0) {
        ...
        for (weight = 0.0, i = 0; i < nsamp; i++) weight += probetbl[i].weight;
        ...
        if (j == nproc - 1) {
            ...
            Error("Only got %d of %d splits\n", j + 1, nproc);

        Free(probetbl);
    } else
        MPMY_Gather(probes, sizeof(Tag_t) * NPROBES, MPMY_CHAR, 0, 0);
