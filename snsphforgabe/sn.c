...
void eosgen_setup(SPHbody *btab, int nobj) {
    ...
    for (p = btab; p < btab + nobj; p++) {
        ...
        Fortran(eosgen)(
            &rho, &temp, &ye, &abar, &u, &u2, &pr, &xp, &xn, &ufreez, &ifleos, &ident, &procnum);
        p->u = u;
        p->u2 = u2;
        p->pr = pr;
        p->xp = xp;
...
void movebound(SPHbody *btab, int nobj, float t, double rb, double *vb, int *icore) {
    ...
    if (rb > 2.5e-4) {
        ...
        *vb = homfac * rb;
        ...
        for (p = btab; p < btab + nobj; p++) { p->r = sqrtf_fast(Dot(p->pos, p->pos)); }
        *vb = 0.0;
    }
    if (*vb >= 0.0)
        *icore = 2;
...
void swerror(void) { Error("error in Fortran space\n"); }

void swerror_(void) { Error("error in Fortran space\n"); }
